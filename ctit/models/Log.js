...
LogDAO.prototype.findByName = function (name, callback) {
    LogModel.findOne({"name": name, "status": constants.CATEGORY_ENABLE_STATUS}, function (err, obj) {
        callback(err, obj);
    });
};
LogDAO.prototype.findById = function (id, callback) {
    LogModel.findOne({"_id": id, "status": constants.CATEGORY_ENABLE_STATUS}, function (err, obj) {
        callback(err, obj);
    });
};

LogDAO.prototype.findByPage = function (page, callback) {
    var query = LogModel.find({"status": constants.CATEGORY_ENABLE_STATUS});
...
LogDAO.prototype.findAll = function (callback) {
    LogModel.find({"status": constants.CATEGORY_ENABLE_STATUS},{},{sort:{"modifyTime":-1}}, function (err, docs) {
        callback(err, docs);
    });
};
LogDAO.prototype.getCount = function (callback) {
    LogModel.count({"status": constants.CATEGORY_ENABLE_STATUS}, function (err, total) {
        callback(err, total);
    });
};
LogDAO.prototype.delete = function (id, callback) {
    LogModel.update({"_id": id}, {$set: {'status': constants.CATEGORY_UNABLE_STATUS }}, function (err) {
        callback(err);
    });
};
LogDAO.prototype.update = function (id, name, callback) {
    LogModel.update({"_id": id}, {$set: {"name": name, "modifyTime": new Date()}}, function (err) {
        callback(err);
    });
};
